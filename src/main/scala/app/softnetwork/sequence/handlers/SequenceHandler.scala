...
trait SequenceTypeKey extends CommandTypeKey[SequenceCommand] {
  override def TypeKey(implicit tTag: ClassTag[SequenceCommand]): EntityTypeKey[SequenceCommand] =
    Sequence.TypeKey
}

trait GenericSequenceHandler
    extends EntityPattern[SequenceCommand, SequenceResult]
    with CommandTypeKey[SequenceCommand] {
...
trait GenericSequenceDao { _: GenericSequenceHandler =>
  ...
  def inc(
    sequence: String
  )(implicit system: ActorSystem[_]): Future[Either[SequenceResult, Int]] = {
  ...
  def reset(
    sequence: String
  )(implicit system: ActorSystem[_]): Future[Either[SequenceResult, SequenceResetted]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
