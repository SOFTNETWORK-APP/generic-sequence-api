...
/** Created by smanciot on 18/03/2020.
  */
trait Sequence
    extends EntityBehavior[SequenceCommand, SequenceState, SequenceEvent, SequenceResult] {
  ...
  override def handleCommand(
    ...
    timers: TimerScheduler[SequenceCommand]
  )(implicit
    context: ActorContext[SequenceCommand]
  ): Effect[SequenceEvent, Option[SequenceState]] = {
    command match {

      case _: IncSequence =>
        Effect
          .persist(SequenceIncremented(entityId, state.map(_.value + 1).getOrElse(1)))
          .thenRun(state => SequenceIncremented(entityId, value(state)) ~> replyTo)

      case _: DecSequence =>
        Effect
          .persist(SequenceDecremented(entityId, state.map(_.value - 1).getOrElse(0)))
          .thenRun(state => SequenceDecremented(entityId, value(state)) ~> replyTo)

      case _: ResetSequence =>
        Effect
          .persist(SequenceResetted(entityId))
  ...
  override def handleEvent(state: Option[SequenceState], event: SequenceEvent)(implicit
